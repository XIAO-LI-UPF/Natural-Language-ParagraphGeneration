...
python IMSurReal/main.py train -m $DIR/$LANG.t1.tsp.mdl -t $T1_TRAIN --d $T1_DEV --task tsp --paragraph $TYPE 
...
## Train T2
## linearization with TSP decoder
python IMSurReal/main.py train -m $DIR/$LANG.t2.tsp.mdl -t $T2_TRAIN --d $T2_DEV --task tsp   --paragraph $TYPE 
# (optional) swap post-processing, for treebanks with many non-projective trees
...
# T1
python IMSurReal/main.py pred  -m $DIR/$LANG.t1.tsp.mdl -i $T1_TEST -p $DIR/$LANG.t1.tsp.conllu --paragraph $TYPE
...
# T2
python IMSurReal/main.py pred  -m $DIR/$LANG.t2.tsp.mdl -i $T2_TEST -p $DIR/$LANG.t2.tsp.conllu --paragraph $TYPE
