    ...
    with (lzma.open(filename, "rt", encoding='utf-8') if filename.endswith('xz') else open(filename)) as fin:
        while True:
            ...
            if line == '' or (first and count >= first): # end of file or reach maximum sentence count
                ...
                if line.startswith('#'):# (# sent_id = en_partut-ud-3)
                    sent.meta += line   #(# text = Distribution of this licens*****)
                    if line.startswith('# text = '):
                        sent.text = line.strip().split('# text = ')[1]
                    if line.startswith('# newpar id ='):
                        sent.newpar_id = line.strip().split('# newpar id =')[1]
                else:
    ...
    with open(filename, 'w') as out:
        ...
        for sent in sents:
            ...
            if header:
                ...
                if hasattr(sent, 'newpar_id'):
                    line += '# newpar id =' + sent.newpar_id + '\n'
                line += f"# text = {text}\n"
            else:
                line = ''
            for t in out_tokens:
...
# input all sentance
def organization_paragraph(sents):
    paragraph = []
    sentens = []
    for i,sent in enumerate(sents):
        if hasattr(sent, 'newpar_id'):
            if len(sentens) > 0:
                paragraph.append(sentens.copy())
                sentens.clear()
            sentens.append(sent)
        elif i == (len(sents) - 1):
            paragraph.append(sentens.copy())
...
def disorganize_paragraph(paragraph):
    sentens = []
...
# def iterate_paragraph(sents):
#     for list in organization_paragraph(sents):
#         yield list

# Change to reading generate paragraph
def iterate_batch(train_sents, extra_sents, size, ratio,paragraph):
    # for batch in paragraph:
    #     yield batch
    # batch = sample(paragraph, size)
    # yield batch
    ...
    while True:
        for i in range(size):
            if not extra_sents or i % (ratio + 1) == 0:
                batch.append(random.choice(paragraph))
            else:
...
# type = all or pre
def get_pervious_sentence(paragraph,index,type='all'):
    if type == 'all':
        return paragraph[0:index]
    elif type == 'pre':
        if index <= 0:
            return []
...
if __name__ == '__main__':
    # simple test 
    sents = list(read_conllu('../data/UD/en_ewt-ud-test.conllu'))
    print(sents)
