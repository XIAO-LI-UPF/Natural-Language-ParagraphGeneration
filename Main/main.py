...
class Realization(object):
    def __init__(self, args):
        ...
        if args.mode == 'train':
            ...
            self.simple_extra_sents = read_conllu(self.args.extra_file, True, skip_lost, self.args.orig_word,
            ...
            self.train_sents = read_conllu(self.args.train_file, False, skip_lost, self.args.orig_word,
                                           self.args.lemmatize, False, False, self.args.first_train)
            self.extra_sents = read_conllu(self.args.extra_file, True, skip_lost, self.args.orig_word,
                                           self.args.lemmatize, False, True, self.args.first_extra) if self.args.extra_file else None
            self.paragraph = organization_paragraph(list(self.train_sents))

            self.iterate_batch = iterate_batch(self.train_sents, self.extra_sents,  self.args.eval_every, self.args.extra_ratio,self.paragraph)
            ...
            self.dev_evalu_sents = disorganize_paragraph(organization_paragraph(self.dev_sents)[:150])
            ...
            self.log(f'train paragraph:{len(self.paragraph)}')
    ...
    def renew_computation_graph(self):
        dy.renew_cg()

    def clear_predict(self,sentence):
    ...
    def train(self):
        ...
        for batch in self.iterate_batch:
            # print(f'The paragraph contains {len(batch)} sentences | {run_count}/{totle} | {run_count/totle*100:.2f}%')
            print(f'Training {len(batch)} paragraph')
            t0 = time()
            for paragraph in tqdm(batch):
                self.renew_computation_graph()
                previous = []
                loss = 0
                for index,sent in enumerate(paragraph):
                    ...
                    for task in self.args.tasks:
                        step += 1
                        if model.args.paragraph != 'skip':
                            previous = get_pervious_sentence(paragraph,index,self.args.paragraph)
                            self.decoders[task].set_previous_sentence(previous)
                ...
                self.clear_predict(paragraph)
                self.clear_predict(previous)
            train_time = time() - t0

            print(f'Training {len(batch)} paragraph Ending - Evaluate on dev data')
            t0 = time()
            res = self.predict_all(self.dev_evalu_sents, pipeline=True)
            test_time = time() - t0
            self.log(f"[step={step}]\ttrain_time={train_time:.1f}s\ttest_time={test_time:.1f}")

            stop = False
            for task in self.args.tasks:
                self.log(f"[step={step}]\t{task}_loss={all_loss[task]/len(self.dev_evalu_sents):.2f}, " \
                ...
                # only start counting after reasonable number of steps
                    ...
                    if waited > self.args.patience:
                        self.log('train out of patience')
                        stop = True
    ...
    def finetune(self, best_scores):
        ...
        for task in self.args.tasks:
            ...
            for batch in self.iterate_batch:
                ...
                # evaluate on dev set
                print('Finetune model: evaluate on dev data')
                res = self.predict(self.dev_evalu_sents, task=task)
                score = res['score']
                self.log(f"[step={step}]\ttrain_time={train_time:.1f}s\ttest_time={res[f'time']:.1f}s")
                self.log(f"[step={step}]\t{task}_loss={all_loss/len(self.dev_evalu_sents):.2f}, " \
    ...
    def predict(self, sents, task):
        ...
        paragraph = organization_paragraph(sents)
        res = defaultdict(float)
        for sentens in tqdm(paragraph):
            self.renew_computation_graph()
            for index,sent in enumerate(sentens):
                self.encode_sent(sent)
                t0 = time()
                if model.args.paragraph != 'skip':
                    self.decoders[task].set_previous_sentence(get_pervious_sentence(sentens,index,self.args.paragraph))
                self.decoders[task].predict(sent)
                res[f'time'] += (time()-t0)
        res[f'score'] = self.decoders[task].evaluate(sents)
        self.clear_predict(sents)
        return res
    ...
    def predict_all(self, sents, pipeline=False, output=False):
        ...
        for sentens in tqdm(paragraph):
            self.renew_computation_graph()
            self.clear_predict(sentens)
            for index, sent in enumerate(sentens):
                self.encode_sent(sent)
                for task in self.args.tasks:
                    t0 = time()
                    if model.args.paragraph != 'skip':
                        previous = get_pervious_sentence(sentens, index, self.args.paragraph)
                        self.decoders[task].set_previous_sentence(previous)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--paragraph",default='skip',choices=['skip','pre','all'], help="generator paragraph add previous single sentence or all sentences")
    parser.add_argument("--hid_dim", type=int, default=128)
    parser.add_argument("--max_step", type=int, default=4000)
    parser.add_argument("--eval_every", type=int, default=300,help='paragraphs per training session')
    ...
    # use the model to predict and write to a output file
        ...
        model.args.paragraph = args.paragraph
