...
def main(UD_file, in_file, out_file, scramble=False, is_test_file=False):
    sent_id = 0
    with open(out_file, 'w') as out:
        out_sents = []
        if is_test_file:
            t_list = read_conllu_list(in_file)
            u_list = read_conllu_list(UD_file)
            for t,u in zip(t_list,u_list):
                sent_id += 1
                for u_sent in u :
                    if '# newpar id' in u_sent:
                        t.insert(0,u_sent)
                for t_sent in t:
                    out_sents.append(t_sent)
        else:
            ...
            for usent, tsent in zip(udsents, t2sents):
                ...
                if out_sent:
                    ...
                    if hasattr(usent, 'newpar_id'):
                        line += '# newpar id =' + usent.newpar_id + '\n'
...
if __name__ == '__main__':
    ...
    is_test_file = (len(sys.argv) > 4 and sys.argv[4] == 'test_file')
    main(sys.argv[1], sys.argv[2], sys.argv[3], scramble,is_test_file)
    # main('../data/UD/en_partut-ud-train.conllu' ,'../data/T1/en_partut-ud-train.conllu', '../data/T1/en_partut-ud-train.gold.conllu')
