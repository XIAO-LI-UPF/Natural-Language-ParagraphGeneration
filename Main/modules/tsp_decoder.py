...
class TSPDecoder(Decoder):
    def __init__(self, args, model, full = False):
        ...
        self.attention = dm.Attention(self.model, self.args.token_dim, self.args.token_dim, self.args.token_dim)
    ...
    def decode(self,tokens, constraints=[], train_mode=False,sent=None):
        loss = 0
        errs = []

        if len(self.previous_sentence):
            current_sent_tsp_tree_vecs = sent.root.vecs['tsp_tree']
            for pre_sent in self.previous_sentence:

                previous_forward = self.f_lstm.initial_state().transduce([t.vecs[self.vec_key] for t in pre_sent['linearized_tokens']])[-1]
                previous_backward = self.b_lstm.initial_state().transduce([t.vecs[self.vec_key] for t in pre_sent['linearized_tokens'][::-1]])[-1]
                tow_way = self.attention.encode(previous_forward, previous_backward)
                init_vec = dy.concatenate([tow_way, self.init_c])
                s = self.lstm_decoder.initial_state().add_input(init_vec)
                enc_vec = self.attention.encode(current_sent_tsp_tree_vecs, s.output())

                for t in tokens:
                    t.vecs[self.vec_key] += enc_vec
    ...
    def set_previous_sentence(self,previous=[]):
        self.previous_sentence = previous
    ...
    def predict(self, sent, pipeline=False):
        ...
        if self.full:
            ...
            res = self.decode(sent[self.pred_input_key], constraints, sent=sent)
            ...
            for token in traverse_bottomup(sent.root):
                ...
                if len(domain) > 1:
                    ...
                    res = self.decode(domain, constraints, sent=sent)
    ...
    def train_one_step(self, sent):
        ...
        if self.full:
            constraints = [] if self.args.no_lin_constraint else self.get_tree_constraints(sent)
            res = self.decode(sent[self.train_input_key], constraints, True, sent)
            loss = res['loss']
            total += 1
            sent['linearized_tokens'] = res['seq']
            ...
            for token in traverse_bottomup(sent.root):
                ...
                if len(domain) > 1:
                    constraints = [] if self.args.no_lin_constraint else self.get_subtree_constraints(token)
                    res = self.decode(domain, constraints, True, sent)
                    token['linearized_domain'] = res['seq']
                    loss += res['loss']
                    total += 1
